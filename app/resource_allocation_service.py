...
# New methods for filtering resources
async def filter_resources_by_skill(skill: str):
    async with get_db() as session:
        result = await session.execute(select(Resource).where(Resource.skill == skill))
        return result.scalars().all()

async def filter_resources_by_project(project: str):
    async with get_db() as session:
        result = await session.execute(select(Resource).where(Resource.project == project))
        return result.scalars().all()

async def filter_resources_by_availability(available: bool):
    async with get_db() as session:
        result = await session.execute(select(Resource).where(Resource.available == available))
        return result.scalars().all()